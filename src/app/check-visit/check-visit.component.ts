...
export class CheckVisitComponent implements OnInit {
  ...
  displayInfo(i: number, j: number) {
    let info = this.receivedUser.paths[i].visitors[j];
    alert('IP Info:\n\nIP address: '+info.ip+'\nVisits: '+info.count+'\nCountry: '+info.geoInfo.country+'\nRegion: '+info.geoInfo.region+'\nCity: '+info.geoInfo.city);
  }
